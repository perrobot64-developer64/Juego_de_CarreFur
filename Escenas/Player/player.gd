...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		rotation_y -= event.relative.x * VariablesGlobales.MOUSE_SENSITIVITY
		rotation_degrees.y = rad_to_deg(rotation_y)

		# Rotación vertical a la cámara (pitch)
		rotation_x = clamp(rotation_x - event.relative.y * VariablesGlobales.MOUSE_SENSITIVITY, deg_to_rad(-80), deg_to_rad(80))
